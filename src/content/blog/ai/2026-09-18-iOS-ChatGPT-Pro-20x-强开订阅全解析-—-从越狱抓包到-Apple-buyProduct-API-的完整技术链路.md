...
categories: system
tags: ["gpt"]
id: "chatgpt-pro-20x-by-ios"
date: 2026-09-18 12:37:08
cover: ""
recommend: false
top: false
---
...
我拿到了那份教程的完整版，六章，从前置条件、网络配置、越狱设置一路讲到拦截原理和字段含义。下面在它的基础上补上我们自己的分析，顺带跟之前拆过的 [Stripe 协议支付](https://blog.caowo.de/posts/stripe-protocol-payment-automation-deep-dive-2026)、[Google Play 订阅转移](https://blog.caowo.de/posts/gpt-plus-google-play-subscription-transfer) 摆在一起对照，把这条链讲透。
...
这和我们之前写过的 [Google Play 订阅转移](https://blog.caowo.de/posts/gpt-plus-google-play-subscription-transfer) 是一个思路：
...
或者更直接的方式——类似我们之前分析过的 [iOS 收据复用漏洞](https://blog.caowo.de/posts/gpt-plus-receipt-vulnerability-2026)：
...
1. **代理 IP 一致性**：这个请求必须走和 Phase 1 相同的出口 IP。Apple 会比对购买请求的源 IP，如果和鉴权生成时的 IP 不同，可能被拒。这与 [Stripe 协议支付](https://blog.caowo.de/posts/stripe-protocol-payment-automation-deep-dive-2026)中的"出口 IP 一致性"规则完全相同。
...
这个方法和我们之前写的 [iOS 收据复用漏洞](https://blog.caowo.de/posts/gpt-plus-receipt-vulnerability-2026) 是可以结合使用的。
...
- [0 PHP 跨区混淆](https://blog.caowo.de/posts/chatgpt-plus-0php-cross-region-pricing-exploit-2026)：**Stripe 创建 session 时的区域信号不一致**
- [prorationMode Hook](https://blog.caowo.de/posts/google-play-ccmax-proration-vulnerability-2026)：**Google Play Billing 的 proration 参数可被客户端修改**
- [RevenueCat 凭证转移](https://blog.caowo.de/posts/gpt-plus-google-play-subscription-transfer)：**RevenueCat 不校验付款人与开通人的对应关系**
- [₱982 Pro 20x TOCTOU](https://blog.caowo.de/posts/chatgpt-pro20x-982php-toctou-race-condition-2026)：**Stripe PI 金额锁定与订阅计划变更不互斥**
- [iOS 收据复用](https://blog.caowo.de/posts/gpt-plus-receipt-vulnerability-2026)：**OpenAI 不校验收据与 Apple ID 的绑定关系**
